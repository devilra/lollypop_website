import React from "react";
import Domains from "./Domains";

const Achievement = () => {
  return (
    <div className="text-black pb-32">
      <div className="px-5 md:px-0 mt-20 md:mt-36 md:ml-32 ">
        <h1 className="text-white md:text-black text-2xl md:text-5xl font-bold">
          Our Achievements
        </h1>
        <p className="pt-5 md:w-[500px] text-xl text-neutral-300 md:text-black md:text-2xl">
          Our design solutions have not only won hearts and people, but also
          many awards and recognitions for exceptional digital experiences.
        </p>
      </div>
      <div className="mt-16 md:ml-36 px-4 w-full md:px-0 grid grid-cols-2 md:gap-6 gap-4 md:grid-cols-3">
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            India's Best Design Projects 2025
          </h1>
          <p className="text-center text-[14px]">
            India's Best Design Award, 2025
          </p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            DNA Paris Design Winner
          </h1>
          <p className="text-center text-[14px]">
            DNA PARIS DESIGN AWARD, 2025
          </p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            Silver A'Design Winner
          </h1>
          <p className="text-center text-[14px]">
            India's Best Design Award, 2025
          </p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            India's Best Design Projects 2025
          </h1>
          <p className="text-center text-[14px]">
            A'DESIGN AWARD & COMPETITION, 2024
          </p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            DNA Paris Design Honorable Mention
          </h1>
          <p className="text-center text-[14px]">
            DNA PARIS DESIGN AWARD, 2024
          </p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            Top B2B Companies
          </h1>
          <p className="text-center text-[14px]">
            CLUTCH GLOBAL SPRING AWARDS, 2024
          </p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            IDA Honorable Mention
          </h1>
          <p className="text-center text-[14px]">
            INTERNATIONAL DESIGN AWARDS, 2023
          </p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            IDA Silver Winner
          </h1>
          <p className="text-center text-[14px]">
            INTERNATIONAL DESIGN AWARDS, 2022
          </p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            DNA Paris Design Winner
          </h1>
          <p className="text-center text-[14px]">
            DNA PARIS DESIGN AWARD, 2022
          </p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            DNA Paris Design Winner
          </h1>
          <p className="text-center text-[14px]">
            DNA PARIS DESIGN AWARD, 2021
          </p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            Top B2B Companies
          </h1>
          <p className="text-center text-[14px]">CLUTCH GLOBAL AWARDS, 2021</p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            DNA Paris Design Winner
          </h1>
          <p className="text-center text-[14px]">
            DNA PARIS DESIGN AWARD, 2020
          </p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            Interface Design Award
          </h1>
          <p className="text-center text-[14px]">RED DOT AWARD, EUROPE, 2019</p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            UX UI Design Leaders
          </h1>
          <p className="text-center text-[14px]">CLUTCH, 2019</p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            Digital Interaction Award
          </h1>
          <p className="text-center text-[14px]">
            DESIGN INTELLIGENCE AWARD, CHINA, 2019
          </p>
        </div>
        <div>
          <img
            src="/studio/award.svg"
            alt="award"
            className="h-8 ml-14 md:ml-32"
          />
          <h1 className="font-bold text-xl text-center py-4">
            Best Design Agency - Gold
          </h1>
          <p className="text-center text-[14px]">DESIGN X AWARDS, 2018</p>
        </div>
      </div>
      <div>
        <h1 className="mt-28 md:w-[600px] md:ml-36 text-4xl font-bold md:text-5xl">
          Hear what our esteemed client has to say about our work.
        </h1>
        <p className="text-xl md:w-[500px] md:ml-36 md:text-2xl py-8">
          Harsh Agarwal, co-founder of Trading Leagues, talks on how we helped
          him launch his business and scale up.
        </p>
        <img
          src="/studio/video.png"
          alt="Video"
          className="h-full md:h-[400px] md:w-[700px] md:ml-36 w-full my-5"
        />
      </div>
      <Domains />
    </div>
  );
};

export default Achievement;
